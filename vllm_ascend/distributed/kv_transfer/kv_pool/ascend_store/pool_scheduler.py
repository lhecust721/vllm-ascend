    ...
    def build_connector_meta(self, scheduler_output: SchedulerOutput) -> KVConnectorMetadata:
        ...
        if not force_skip_save:
            for i, req_id in enumerate(cached_reqs.req_ids):
                ...
                # Autoregressive Decode normally reuses an existing physical
                # block, so no new block IDs are returned. Still process the
                # step when Decode cache saving is enabled, otherwise the
                # tracker never reaches the next transfer boundary.
                track_decode = self.save_decode_cache and self._is_decoding_request(req_id)
                if not new_block_ids and not track_decode:
                    continue
